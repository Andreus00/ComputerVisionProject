...
from src.guidance.zero123_utils import Zero123
import numpy as np
...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, path="./out"):
    ...
    scene = Scene(dataset, gaussians, model_path=path)
    gaussians.training_setup(opt)
    guidance = Zero123(device="cuda", fp16=True, t_range=[0.02, 0.98], model_key="ashawkey/zero123-xl-diffusers")
    if checkpoint:
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        if not viewpoint_idx_list:
            viewpoint_stack = scene.getTrainCameras().copy()
            viewpoint_idx_list = list(range(len(viewpoint_stack)))
            input_img = torch.asarray(viewpoint_stack[0].original_image).unsqueeze(0)
            if iteration <= 1:
                guidance.get_img_embeds(input_img)

        random_cam_idx = randint(0, len(viewpoint_idx_list)-1)
        # random_cam_idx = 0
        el = viewpoint_idx_list.pop(random_cam_idx)
        # el = 0
        viewpoint_cam = viewpoint_stack[el]
        ...
        if iteration > 500:
            ...
            with torch.no_grad():
                    # render random view
                ver = np.random.randint(-20, 20)
                hor = np.random.randint(-180, 180)
                radius = 10
                c2w = orbit_camera(ver, hor, radius=radius, is_degree=True, target=None, opengl=True)
                c2w[:3, 1:3] *= -1
...
def main(path):
    ...
    parser.add_argument('--debug_from', type=int, default=-1)
    parser.add_argument('--detect_anomaly', action='store_true', default=False)
    parser.add_argument("--test_iterations", nargs="+", type=int, default=[300, 500, 1_000, 2_000, 4_000, 7_000, 10_000])#, 30_000])
    parser.add_argument("--save_iterations", nargs="+", type=int, default=[300, 500, 1_000, 2_000, 4_000,  7_000, 10_000])#, 30_000])
    parser.add_argument("--quiet", action="store_true")
    parser.add_argument("--checkpoint_iterations", nargs="+", type=int, default=[])
    parser.add_argument("--start_checkpoint", type=str, default = None)
    args = parser.parse_args(sys.argv[1:])
    ...
    network_gui.init(args.ip, args.port)
    torch.autograd.set_detect_anomaly(args.detect_anomaly)
    training(lp.extract(args), op.extract(args), pp.extract(args), args.test_iterations, args.save_iterations, args.checkpoint_iterations, args.start_checkpoint, args.debug_from, path)
