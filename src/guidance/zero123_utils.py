...
class Zero123(nn.Module):
    ...
    def train_step(self, pred_rgb, elevation, azimuth, radius, step_ratio=None, guidance_scale=5, as_latent=False, default_elevation=0):
        ...
        if as_latent:
            ...
            pred_rgb_256 = F.interpolate(pred_rgb, (256, 256), mode='bilinear', align_corners=True)
            latents = self.encode_imgs(pred_rgb_256.to(self.dtype))
        ...
        # with torch.no_grad():
        #     reconstructed_latent = self.scheduler.step(noise_pred, t_in[0].item(), latents).prev_sample.to("cuda")
        #     decoded_img = self.decode_latents(reconstructed_latent.detach())
        #     cv2.imwrite(f"reconstructed_image.png", np.flip(decoded_img[0].permute(1, 2, 0).cpu().numpy(), (2)) * 255)
